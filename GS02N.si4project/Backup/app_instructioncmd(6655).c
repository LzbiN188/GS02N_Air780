...
static void doSetBatRateInstruction(ITEM *item, char *message)
{
	if (item->item_data[1][0] == 0 || item->item_data[1][0] == '?')
	{
		sprintf(message, "Bat high rate is %.2f, low rate is %.2f", sysparam.batHighLevel, sysparam.batLowLevel);
	}
	else
	{
		sysparam.batHighLevel = atof(item->item_data[1]);
		sysparam.batLowLevel = atof(item->item_data[2]);
		sprintf(message, "Update bat high rate to %.2f, low rate to %.2f", sysparam.batHighLevel, sysparam.batLowLevel);
		paramSaveAll();
	}
}
/*--------------------------------------------------------------------------------------*/
